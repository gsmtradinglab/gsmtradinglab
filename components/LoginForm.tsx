...
export default function LoginForm(){const r=useRouter(); const [email,setEmail]=useState(""); const [password,setPassword]=useState(""); const [msg,setMsg]=useState("");
 async function login(e:React.FormEvent){e.preventDefault(); try{await signInWithEmailAndPassword(auth,email,password); r.push("/dashboard")}catch(err:any){setMsg(err.message)}}
 async function google(){await signInWithPopup(auth,googleProvider); r.push("/dashboard")}
 async function reset(){if(!email) return setMsg("Enter email first"); await sendPasswordResetEmail(auth,email); setMsg("Password reset email sent")}
