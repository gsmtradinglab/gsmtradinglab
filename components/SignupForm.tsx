...
export default function SignupForm(){
 ...
 async function submit(e:React.FormEvent){e.preventDefault(); setMsg("Creating account..."); try{if(!firebaseConfigured||!auth||!db) throw new Error("Firebase environment variables are missing. Add NEXT_PUBLIC_FIREBASE_* keys in Vercel."); const c=await createUserWithEmailAndPassword(auth,form.email,form.password); await updateProfile(c.user,{displayName:form.fullName}); await saveUser(c.user.uid,form.email,form.fullName); r.push("/dashboard");}catch(err:any){setMsg(err.message)}}
 async function google(){if(!firebaseConfigured||!auth||!db) return setMsg("Firebase environment variables are missing. Add NEXT_PUBLIC_FIREBASE_* keys in Vercel."); const c=await signInWithPopup(auth,googleProvider); await saveUser(c.user.uid,c.user.email||"",c.user.displayName||"GSM User"); r.push("/dashboard")}
