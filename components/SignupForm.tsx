...
export default function SignupForm(){
 ...
 async function saveUser(uid:string,email:string,name:string){const now=new Date().toISOString(); await setDoc(doc(db,"users",uid),{uid,fullName:name,email,phone:form.phone,country:form.country,city:form.city,role:"user",status:"active",courseAccess:"none",paymentStatus:"none",createdAt:now,updatedAt:now},{merge:true});}
 async function submit(e:React.FormEvent){e.preventDefault(); setMsg("Creating account..."); try{const c=await createUserWithEmailAndPassword(auth,form.email,form.password); await updateProfile(c.user,{displayName:form.fullName}); await saveUser(c.user.uid,form.email,form.fullName); r.push("/dashboard");}catch(err:any){setMsg(err.message)}}
 async function google(){const c=await signInWithPopup(auth,googleProvider); await saveUser(c.user.uid,c.user.email||"",c.user.displayName||"GSM User"); r.push("/dashboard")}
