...
export default function Admin() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <p className="mt-2 text-slate-400">Manage users, registrations, payments, premium access, and signals.</p>

      <div className="mt-8 flex flex-wrap gap-2">
        {tabs.map((x) => (
          <button key={x} onClick={() => setTab(x)} className={tab === x ? "btn-green" : "btn-dark"}>
      ...
      {tab === "overview" && (
        <section className="mt-8 grid gap-5 md:grid-cols-6">
          <div className="card"><b>Total Users</b><p className="text-3xl text-green-400">{users.length}</p></div>
          <div className="card"><b>Premium</b><p className="text-3xl text-green-400">{stats.premium}</p></div>
          <div className="card"><b>Registrations</b><p className="text-3xl text-green-400">{regs.length}</p></div>
          <div className="card"><b>Pending Payments</b><p className="text-3xl text-green-400">{stats.pending}</p></div>
          <div className="card"><b>Verified Payments</b><p className="text-3xl text-green-400">{stats.verified}</p></div>
          <div className="card"><b>Signals</b><p className="text-3xl text-green-400">{signals.length}</p></div>
        ...
        <section className="mt-8">
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-800">
            <table className="w-full text-sm">
              <thead className="bg-slate-900"><tr><th className="p-3 text-left">Name</th><th>Email</th><th>Role</th><th>Status</th><th>Access</th><th>Payment</th><th>Actions</th></tr></thead>
              <tbody>
                {filteredUsers.map((u) => (
